...
//    },[props.submitted])
    ...
    const newObj=LanguageFactory(props.language, props.lang_level);
       ...
       if(newObj.level===undefined){
           console.log('1')
            ...
            if(languages.length>0 && newObj.language===languages[languages.length-1].language){
            console.log('2')
            }
            else{
 ...
 useEffect(()=>{
   const newEmpl=EmploymentFactory(props.company, props.position, props.date_start, props.date_end);
   if(newEmpl.company===undefined){
        console.log(1);
   }

   else{
       if(employment.length>0 && newEmpl.company===employment[employment.length-1].company && newEmpl.position===employment[employment.length-1].position){
           console.log(2)
       }
       else{
 ...
 const Delete=(ev)=>{
     ...
     if(found2!==(-1)){
         setEmployment(employment.filter(element=> element.id!==ev.target.id));
     }
  ...
  Component.defaultProps={
    ...
    return <div className='preview-box' style={styles.bigBox} id='box'>
             <div className='white-box'>
                <nav className='prev-header'>
                    <Upload1/>
                     <div className='prev-header-small'>
                        <img style={styles.logo} src={process.env.PUBLIC_URL+logo} crossOrigin='anonymous'/>
                        <div className='personal'>
                            <h2 name='name'>{props.name} {props.surname}</h2>
                ...
                <div className='prev-item'>
                    ...
                    { 
                        education.map((obj)=>{
                            ...
                            return   <div className='div-item' key={obj.id}>
                                        <h2 className='div-heading' name='edu-level'>{obj.level}</h2>
                                        <h2 name='institiution'>{obj.institution}</h2>                                        
                                        <h2 name='major'>{obj.major}</h2>
                                        <h2  className='date' name='year'>{obj.inroll} - { obj.year}</h2>                                   
                                        
                                        
                                        <img className='del-icon' id={obj.id} onClick={Delete} style={styles.icon} src={process.env.PUBLIC_URL+'./trash.svg'}/>
                                    </div>
                        })
                    }
                    
                     
                      <PrevEdu level={props.level} institiution={props.institiution} inroll={props.inroll} year={props.year} major={props.major} />
                ...
                <h1 style={styles.h1}> Previous Employment</h1>
                 <div className='prev-item'>
                    
                     {
                        employment.map((obj)=>{
                            return   <div className='div-item' key={obj.id}>
                                        ...
                                        <h2 className='date'>{obj.start} - {obj.end}</h2>
                                        
                                        <FontAwesomeIcon id={obj.id} onClick={Delete} icon="fa-solid fa-circle-trash" />
                                    </div>
                            })
                    }
                    <div className='div-item'>
                        ...
                        <h2 className='date' >{props.date_start} - {props.date_end}</h2>
                    </div>
                   ...
                   
                    {
                        languages.map((obj)=>{
                            return   <div className='div-item' key={obj.id}>
                                        ...
                                        <button id={obj.id} onClick={Delete}> DELETE</button>
